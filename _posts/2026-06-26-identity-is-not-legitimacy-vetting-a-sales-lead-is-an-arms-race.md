...
This is real and tested, just not novel. It is essentially Palisade Research's [LLM Agent Honeypot](https://arxiv.org/abs/2410.13919), which combines prompt-injection traps with response-time analysis and, over a few months, fingerprinted live AI agents out of millions of attempts. Cloudflare's [AI Labyrinth](https://blog.cloudflare.com/ai-labyrinth/) industrializes the hidden-link version of the same idea. The problem here is that hardened agents already filter out fields with suspicious styling and skip them, and a [2025 paper on LLMs polluting online research](https://arxiv.org/pdf/2508.01390) recommends prompt injections only as hidden honeypot questions for exactly that reason. All it really buys you is a higher cost to the attacker.
...
Nothing real gets hidden. The score lands as a sortable, filterable column right inside the CRM, whether that is Salesforce or HubSpot, so the sales team still sees the low scorers alongside everything else. A rep who wants to open a low-scored lead and chase the whale anyway can do exactly that. The score orders the pile; it does not make decisions for the person working it.
