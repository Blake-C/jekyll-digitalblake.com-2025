...
I also didn’t want to have to remember the command to quickly kill the desktop in macOS so I created a quick shell script where I can just call `desktop` in my terminal and tell it to hide; then get on with my training. Run the command again and tell the script to show the desktop when you’re done.
