...
First, in the `what_node` function I use [ripgrep](https://github.com/BurntSushi/ripgrep) (rg) to find the line in the `package.json` file that holds the node version. Note I’m only looking in the current directory as some sub directories could have their own `package.json` files.
...
Next is to create a function that will take our node version and switch us over to it using [nvm](https://github.com/nvm-sh/nvm). Because `nvm` might not have the exact version of node I need for this project I `awk` the node version to get the major version number and save this in a variable.
