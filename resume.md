...
After leaving Seismic as part of a reduction in force (RIF) and taking some time to recharge, I've been working on coming up to speed on AI tooling using Claude Code for several integrations and projects, and then also utilizing those same skills on a coding challenge. I've continued to do my research in utilizing this new tool chain in preparation for my next role.

I've redeveloped my starter framework for building WordPress websites, built an exit intent pop-up plugin for capturing leads that would leave a request-a-demo form, and built a Claude code overwrite extension to optimize features to work the way I need. Check the [articles](/articles/) page where you'll find my thoughts on LLM writing, challenges building with Sanity and NextJS, and what to look out for when building sites that need to be WCAG conformant.
