...
- **Critical CSS must import layout partials in the same order as `global-styles.scss`.** Both emit the same selectors at equal specificity, so source order decides the cascade, and a mismatch makes the page resolve one way at first paint and another once the deferred stylesheet lands. `test/content.test.mjs` enforces it; `test/e2e/first-paint.spec.mjs` catches the symptom.
...
**Case study ordering** is controlled by an `order:` integer in each case study's front matter, not the filename prefix. `_includes/case-studies.html` (and the "More Case Studies" block in `_layouts/case-study.html`) sort by `order` descending, so a higher number sits higher on the page. Featured entries and the rest (`featured: false`, shown under the "Additional Case Studies" heading) render as separate grids but share this one sort. Existing values step by 10 (`140` down to `10`).

The values are sequenced newest first by the `year:` in each entry's front matter, so the two grids read as one descending timeline. A range like `2022 to 2025` sorts on its end year, and entries sharing a year keep the relative order they already had. Seismic is the one exception, held above Teleport Atlas so it leads the featured row even though Teleport Atlas is the more recent entry. Adding a case study means picking the `order` that puts it in its chronological slot and renumbering the entries below it, since the sort is on `order` and not on `year`. Case study filenames are just the slug (e.g. `teleport-atlas.md`), with no numeric prefix.

`year:` is a free-text string, rendered next to the card title in both grids and in the "More Case Studies" block. It is optional in the templates, so an entry without one renders a card with no year.
...
`build:fonts` also keeps every character it finds in the content it scans, on top of a fixed unicode range. That scan covers published content only. `SCAN_ROOT_SKIP` in `script/build-fonts.mjs` holds the root markdown files that `_config.yml` excludes from the site, currently `CLAUDE.md` and `README.md`. Without it, a character typed into this file rewrote the WOFF2 and changed its hashed URL, so every visitor re-downloaded the font for a glyph the site never renders. Add a root doc file to that set when you add one to `exclude:`. A new post that uses a character outside the fixed range still changes the font, and that one is correct.
